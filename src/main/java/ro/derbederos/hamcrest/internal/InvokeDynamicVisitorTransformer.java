...
class InvokeDynamicVisitorTransformer implements ClassFileTransformer {
    @Override
    public byte[] transform(final ClassLoader classLoader, String className, Class<?> classBeingRedefined, ProtectionDomain protectionDomain, byte[] classFileBuffer) throws IllegalClassFormatException {
        if (className.endsWith("AnimalSnifferTest")) {
            ClassReader classReader = new ClassReader(classFileBuffer);
            classReader.accept(new InvokeDynamicClassVisitor(Opcodes.ASM5), ClassReader.EXPAND_FRAMES);
        }
        return null;
    }
}